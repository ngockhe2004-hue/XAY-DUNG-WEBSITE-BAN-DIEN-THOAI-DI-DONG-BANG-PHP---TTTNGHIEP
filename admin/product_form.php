...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $slug = trim($_POST['slug'] ?? '') ?: generateSlug($ten_sanpham);
    ...
    try {
        ...
        // --- Xử lý hình ảnh ---
        // Gán biến thể cho ảnh (nếu admin chọn)
        if (!empty($_POST['image_variant'])) {
            foreach ($_POST['image_variant'] as $imgId => $bienthetId) {
                $bienthetId = $bienthetId === '' ? null : (int)$bienthetId;
                db()->execute("UPDATE hinhanh_sanpham SET ma_bienthe = ? WHERE ma_anh = ? AND ma_sanpham = ?",
                    [$bienthetId, (int)$imgId, $productId]);
            }
        }
        ...
        if (!empty($_FILES['hinh_anh']['name'][0])) {
            foreach ($_FILES['hinh_anh']['tmp_name'] as $i => $tmp) {
                ...
                if (move_uploaded_file($tmp, UPLOAD_DIR . $fname)) {
                    $isMain = ($id == 0 && $i === 0) ? 1 : 0;
                    db()->insert("INSERT INTO hinhanh_sanpham (ma_sanpham, image_url, la_anh_chinh) VALUES (?,?,?)",
                        [$productId, $fname, $isMain]);
                }
            }
        }
                    ...
                    <div class="gallery-grid" id="galleryContainer">
                        ...
                        <div class="gallery-item-card <?= $img['la_anh_chinh'] ? 'is-main' : '' ?>" id="img_card_<?= $img['ma_anh'] ?>">
                            ...
                            <div style="padding: 10px; background: var(--card2);">
                                <select name="image_variant[<?= $img['ma_anh'] ?>]" class="form-control" style="font-size: 10px; padding: 5px; height: auto;">
                                ...
                                <label style="display: flex; align-items: center; gap: 8px; margin-top: 8px; cursor: pointer;">
                                    <input type="radio" name="main_image_id" value="<?= $img['ma_anh'] ?>" <?= $img['la_anh_chinh'] ? 'checked' : '' ?> onchange="updateMainPreview(this, '<?= BASE_URL ?>/uploads/products/<?= $img['image_url'] ?>')">
                                    <span style="font-size: 10px; font-weight: 700; color: var(--txt3);">CHÍNH</span>
                                </label>
                            </div>
                        </div>
                        <?php endforeach; ?>
...
<script>
    ...
    function updateMainPreview(radio, url) {
        ...
        document.querySelectorAll('.gallery-item-card').forEach(card => card.classList.remove('is-main'));
        radio.closest('.gallery-item-card').classList.add('is-main');
    }
                ...
                reader.onload = function(e) {
                    ...
                    div.innerHTML = `
                        ...
                        <div style="padding: 10px; background: var(--purple-light);">
                            <div style="font-size: 10px; font-weight: 800; color: var(--purple); text-align: center;">ẢNH MỚI CHỜ TẢI</div>
                        </div>
                    `;
